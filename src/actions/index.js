...
export const preview_receive = ({ source, id, text }) => ({ type: 'preview_receive', payload: { source, id, text } })
...
export const preview = (source, id) => async (dispatch) => {
    const resp = await fetch(`http://localhost:8080/sources/${source}/docs/${id}/text`)
    const reader = await resp.body.getReader()
    const { value } = await reader.read()
    const text = new TextDecoder("utf-8").decode(value);
    return dispatch(preview_receive({ source, id, text }))
